...
const { getQuizInfo, createQuiz } = api;
...
const clearSelections = (index) => {
  if (quizData.question[index].selection_type == "textarea") {
    quizData.question[index].selection = [""];
  }
};
...
const selectionsToString = (index) => {
  //除去選項內容沒有輸入的項目
  const filteredSelections = quizData.question[index].selection.filter(
    (item) => item != ""
  );
...
const deleteQuestions = (index) => {
  quizData.question.splice(index, 1);
};

/* 選項+1 */
const addSelections = (index) => {
  quizData.question[index].selection.push("");
};

/* 刪除指定選項 */
const deleteSelections = (index, seleindex) => {
  quizData.question[index].selection.splice(seleindex, 1);
};
...
const saveQuiz = () => {
  ...
  console.log(quizData.quiz.title);
  // 打開彈跳視窗
  openSavePop();

  // 將選項內容從arr轉換str
  quizData.question.forEach((item, index) => {
    //將req中的selection轉換為字串
    const seleStr = selectionsToString(index);
    quizData.question[index].selection = seleStr;
    console.log(
      "第" + (index + 1) + "題選項" + quizData.question[index].selection
    );
  });

  //state設為: 尚未開始
  quizData.quiz.state = "尚未開始";
...
const clickPostBtn = () => {
  ...
  if (data == true) {
    ...
    quizData.question.forEach((item) => {
      if (item.q_name == "") {
...
const postQuiz = () => {
  ...
  let data = true;
  //先將日期轉為Date資料
  quizData.quiz.start_time = new Date(startDate);
  quizData.quiz.end_time = new Date(endDate);

  // 將日期轉換為日期字串（僅包含日期部分）
  if (!isNaN(quizData.quiz.start_time) && !isNaN(quizData.quiz.end_time)) {
    const startDateStr = new Date(quizData.quiz.start_time)
      .toISOString()
      .split("T")[0];
    ...
    if (startDateStr < currentDateTimeStr) {
      alert("發布日期不能小於今天");
      data = false;
    } else if (quizData.quiz.start_time > quizData.quiz.end_time) {
      alert("發布日期不能大於結束日期");
      data = false;
    } else if (
      quizData.quiz.start_time.getTime() == quizData.quiz.end_time.getTime()
    ) {
      alert("發布日期不能等於結束日期");
      data = false;
    }
  ...
  /* 用日期判斷state狀態 */
  if (quizData.quiz.start_time <= currentDateTime) {
    quizData.quiz.state = "發布中";
  }
...
<template>
  ...
  <div class="body">
    <div class="questionTitle">
      <label for="">標題</label>
      <input
        v-model.trim="quizData.quiz.title"
        type="text"
        placeholder="輸入問卷名稱"
      />
    </div>
    ...
    <div class="quizDesp">
      ...
      <textarea
        v-model.trim="quizData.quiz.description"
        type="text"
        ...
        <div class="setQuestion">
          ...
          <div class="setQues">
            <div class="setQName">
              ...
              <input
                v-model.trim="quizData.question[index].q_name"
                type="text"
                placeholder="輸入問題名稱"
              />
            </div>
            ...
            <div class="setQType">
              ...
              <select
                v-model="quizData.question[index].selection_type"
          ...
          <div class="setSelection">
            ...
            >
              ...
              >
                ...
                <input
                  v-model.trim="quizData.question[index].selection[seleindex]"
                  type="text"
                  placeholder="輸入選項內容"
                />
                <i
